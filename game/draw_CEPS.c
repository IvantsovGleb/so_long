...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/03/21 13:37:52 by fsinestr          #+#    #+#             */
/*   Updated: 2022/03/21 15:03:24 by fsinestr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (p_lst)
	{
		if (!((t_cl *)p_lst->content)->collected)
			mlx_put_image_to_window(
				game->mlx, game->win, game->coll.curr_img,
				((t_cl *)p_lst->content)->pos.x + 15,
				((t_cl *)p_lst->content)->pos.y + 15);
		p_lst = p_lst->next;
	}
}
...
void	draw_exit(t_game *game)
{
	mlx_put_image_to_window(
		game->mlx, game->win,
		game->exit.ladder, game->exit.pos.x, game->exit.pos.y);
	if (game->exit.pos.y == game->win_sz.y - 2 * TILE_SIZE)
	{
		mlx_put_image_to_window(
			game->mlx, game->win,
			game->wall.wall_top, game->exit.pos.x, game->exit.pos.y);
	}
}
...
void	draw_mob(t_game *game)
{
	...
	while (p_lst)
	{
		mlx_put_image_to_window(game->mlx, game->win, game->npc.curr_img,
			((t_point *)p_lst->content)->x + 15,
			((t_point *)p_lst->content)->y + 15);
		p_lst = p_lst->next;
	}
	if (game->player.state == ALIVE && game->player.visible == TRUE)
	{
		...
		mlx_put_image_to_window(game->mlx, game->win, game->player.curr_img,
			game->player.coords.x + 13,
			game->player.coords.y - 15);
	}
	else if (game->player.visible == TRUE)
	{
		update_plr_dead_animation(&game->player);
		mlx_put_image_to_window(game->mlx, game->win, game->player.curr_img,
			game->player.coords.x + 15,
			game->player.coords.y + 15);
	}
}
